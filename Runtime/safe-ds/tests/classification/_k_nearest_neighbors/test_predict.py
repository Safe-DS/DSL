...
def test_k_nearest_neighbors_predict():
...
def test_k_nearest_neighbors_predict_not_fitted():
    table = Table.from_csv("tests/resources/test_k_nearest_neighbors.csv")
    supervised_dataset = SupervisedDataset(table, "T")
    k_nearest_neighbors = KNearestNeighborsClassifier(2)
...
def test_k_nearest_neighbors_predict_invalid():
    table = Table.from_csv("tests/resources/test_k_nearest_neighbors.csv")
...
def test_k_nearest_neighbors_predict_invalid_target_predictions():
