import pandas as pd
from safe_ds.data import OrdinalEncoder, Table, StringColumnType


def test_inverse_transform() -> None:
    test_table = Table(
        pd.DataFrame({"temperatur": [1, 0, 0, 1, 2],
                      "gedöns": ["1", "2", "3", "4", "5"],
    ...
    assert isinstance(test_table.schema.get_type_of_column("temperatur"), StringColumnType)
    assert isinstance(test_table.schema.get_type_of_column("temperatur_2"), StringColumnType)
