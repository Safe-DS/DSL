...
def plot_boxplot(column: Column):
    ...
    for data in column._data:
        if not isinstance(data, int) and not isinstance(data, float) and not isinstance(data, complex):
            raise TypeError("The column contains non numerical data. Boxplots can only plot numerical data")
        if not isinstance(data, int) and not isinstance(data, float):
            raise TypeError("The column contains complex data. Boxplots cannot plot the imaginary part of complex "
