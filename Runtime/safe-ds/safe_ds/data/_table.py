...
class Table:
    ...
    def from_rows(rows: list[Row]) -> Table:
        ...
        dataframe.columns = schema_compare._get_columns()
    ...
    def to_json(self, path_to_file: str) -> None:
        ...
        data_to_json.columns = self.schema._get_columns()
    ...
    def to_csv(self, path_to_file: str) -> None:
        ...
        data_to_csv.columns = self.schema._get_columns()
    ...
    def rename_column(self, old_name: str, new_name: str) -> Table:
        ...
        """
        if old_name not in self.schema._get_columns():
            raise UnknownColumnNameError([old_name])
        if old_name == new_name:
            return self
        if new_name in self.schema._get_columns():
            raise DuplicateColumnNameError(new_name)

        new_df = self._data.copy()
        new_df.columns = self.schema._get_columns()
    ...
    def to_columns(self) -> list[Column]:
        ...
        """
        return [self.get_column(name) for name in self.schema._get_columns()]
    ...
    def drop_duplicate_rows(self) -> Table:
        ...
        df.columns = self.schema._get_columns()
        return Table(df)
    ...
    def replace_column(self, old_column_name: str, new_column: Column) -> Table:
        ...
        if old_column_name != new_column.name:
            ...
            result.columns = renamed_table.schema._get_columns()
        else:
            result = self._data.copy()
            result.columns = self.schema._get_columns()
