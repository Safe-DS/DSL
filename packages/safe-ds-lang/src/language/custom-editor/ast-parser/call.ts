...
export class Call {
    ...
    public static parse(node: SdsCall): Call | CustomError {
        ...
        if (!isValidCallReceiver(node.receiver)) {
            return Utils.pushError(`Invalid Call receiver: ${debugInvalidCallReceiver(node.receiver)}`, node.receiver);
        }
        ...
        argumentList = filterErrors(node.argumentList.arguments.map(Argument.parse));
        ...
        if (isSdsMemberAccess(node.receiver)) {
            ...
            category = Utils.safeDsServices.builtins.Annotations.getCategory(functionDeclaration)?.name.split("Q")[0] ?? '';
            ...
            resultList = filterErrors((functionDeclaration.resultList?.results ?? []).map(Result.parse));
            parameterList = filterErrors((functionDeclaration.parameterList?.parameters ?? []).map(Parameter.parse));
        }
        ...
        if (isSdsReference(node.receiver) && isSdsClass(node.receiver.target.ref)) {
            ...
            if (!classDeclaration.parameterList)
                return Utils.pushError('Missing constructor parameters', classDeclaration);
            parameterList = filterErrors(classDeclaration.parameterList.parameters.map(Parameter.parse));
        ...
        if (isSdsReference(node.receiver) && isSdsSegment(node.receiver.target.ref)) {
            ...
            resultList = filterErrors((segmentDeclaration.resultList?.results ?? []).map(Result.parse));
            parameterList = filterErrors((segmentDeclaration.parameterList?.parameters ?? []).map(Parameter.parse));
        }

        const parameterListCompleted = matchArgumentsToParameter(parameterList, argumentList, node, id);
    ...
    private static parseSelf(node: CallReceiver, id: number) {
        if (isSdsMemberAccess(node)) {
            if (isSdsCall(node.receiver)) {
                ...
                if (call.resultList.length > 1) return Utils.pushError('To many result', node.receiver);
                if (call.resultList.length < 1) return Utils.pushError('Missing result', node.receiver);

                Edge.create(Port.fromResult(call.resultList[0]!, call.id), Port.fromName(id, 'self'));
                ...
                if (isSdsClass(receiver)) {
                    ...
                    Edge.create(Port.fromPlaceholder(placeholder, false), Port.fromName(id, 'self'));
                }
            } else if (isSdsMemberAccess(node.receiver)) {
                const receiver = node.receiver;
                const placeholder = Placeholder.parse(receiver.receiver.target.ref);
    ...
    for (const [index, parameter] of parameterList.entries()) {
        ...
        if (argument) {
            ...
            if (argument.reference instanceof Call) {
                ...
                if (call.resultList.length !== 1) return Utils.pushError('Type missmatch', callNode.argumentList);
                Edge.create(Port.fromResult(call.resultList[0]!, call.id), Port.fromParameter(parameter, id));
            }
            if (argument.reference instanceof GenericExpression) {
                const experession = argument.reference;
                Edge.create(Port.fromGenericExpression(experession, false), Port.fromParameter(parameter, id));
            }
            if (argument.reference instanceof Placeholder) {
                const placeholder = argument.reference;
                Edge.create(Port.fromPlaceholder(placeholder, false), Port.fromParameter(parameter, id));
            }
            if (argument.reference instanceof Parameter) {
                const segmentParameter = argument.reference;
                Edge.create(Port.fromParameter(segmentParameter, -1), Port.fromParameter(parameter, id));
            }
            continue;
        }
        ...
        if (!argument && !parameter.defaultValue) {
            return Utils.pushError(`Missing Argument for ${parameter.name}`, callNode);
        }
    }
