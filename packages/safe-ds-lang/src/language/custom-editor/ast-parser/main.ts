...
import { isSdsPipeline, isSdsSegment, SdsModule, SdsStatement } from '../../generated/ast.js';
...
export const parseDocument = (document: LangiumDocument<AstNode>): [Graph, CustomError[], Segment[]] => {
    Utils.initialize();
    ...
    if (pipelines.length !== 1) {
        return [new Graph('pipeline', Utils.collectAst()), Utils.errorList, segmentList];
    }
    ...
    const graph: Graph = new Graph(
        'pipeline',
        Utils.collectAst(),
        Utils.safeDsServices.workspace.AstNodeLocator.getAstNodePath(pipelines[0]!),
        pipelines[0]!.name,
    );
