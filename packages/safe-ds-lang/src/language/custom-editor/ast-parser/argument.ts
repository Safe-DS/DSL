import { isSdsList, isSdsLiteral, SdsArgument } from '../../generated/ast.js';
...
export class Argument {
    ...
    public static parse(node: SdsArgument) {
        if (!node.value.$cstNode) return Utils.pushError('CstNode missing', node.value);
        const text = node.value.$cstNode.text;

        let expression;
        if (!isSdsLiteral(node.value) || isSdsList(node.value)) expression = Expression.parse(node.value);
        if (expression instanceof CustomError) return expression;

        if (node.parameter && !node.parameter.ref) return Utils.pushError('Missing Parameterreference', node);
