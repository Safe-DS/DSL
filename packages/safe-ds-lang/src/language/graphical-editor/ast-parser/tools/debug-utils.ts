...
import { AstNode, LangiumDocument, URI, isAstNode, isReference } from 'langium';
import { SafeDsAstReflection, isSdsAnnotationCallList } from '../../../generated/ast.js';
...
export const saveJson = (json: {}, documentPath: URI) => {
    const extension = '.txt';
    const basePath = documentPath.fsPath.split('.')[0];
    const path = `${basePath}_debug${extension}`;
...
export const documentToJson = (document: LangiumDocument<AstNode>, depth: number): {} => {
...
export const nodeToJson = (node: AstNode, depth: number): {} => {
    ...
    metadata.properties.forEach((property) => {
        const element = (node as any)[property.name] ?? property.defaultValue ?? '';
